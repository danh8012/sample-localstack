...
docker container run --privileged -tid --publish 8080:8080 --publish 4510-4559:4510-4559 -v ./volume:/var/lib/localstack -v /var/run/docker.sock:/var/run/docker.sock -e DEBUG=1 -e REGION_LOCAL=my-local -e ENV_DEV=local -e IMAGE_NAME=haock_localstack_image -e MAIN_CONTAINER_NAME=haock_localstack_container -e LS_LOG=trace --name haock_localstack_container haock_localstack_image
